...
| IA-08-FE | Agente 5: **frontend admin Angular** — UI en `/admin` para consumir los 4 endpoints de IA-08 (checklist KYB con badges por documento, prevalidación de tour con lista de issues por severidad, borrador DIMAR con export a CSV/PDF, lista de anomalías de payout con drill-down por orden). Bloqueado por IA-08 (ya cerrado) — desbloquear en el próximo sprint FE admin | P2 | S | IA-08 | [16](16-agentes-ia.md) §Agente 5 |
...
| IA-10-FE | Admin UI Angular para la cola de moderación de reseñas — consumir `GET /admin/reviews/moderation` con filtros por status + rango + botón "re-moderate" que llama `POST /admin/agents/review-moderation/{id}/re-moderate` inline + tabla con reviewText preview + link al detalle de la reseña. Sidebar admin bajo "Reseñas". BackofficeGuard (ADMIN + BACKOFFICE_OPERATION). Sin bloqueo — el backoffice puede consumir el endpoint directo mientras tanto. Bloqueado por IA-10 (backend cerrado) | P3 | S | IA-10 | [16](16-agentes-ia.md) §Agente 6 |
