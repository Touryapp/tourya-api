...
| MO-40b | **Deuda hotfix #179**: mover hooks push del `PaymentService` (y demás flows Fase D que corren dentro de tx) a `@TransactionalEventListener(phase = AFTER_COMMIT)`. Objetivo: garantizar que ningún fallo del stack de push (DB, red a FCM, dependencias) pueda tumbar la transacción de negocio. Chequear también: 7 reservas TEMPORAL potencialmente huérfanas entre 2026-07-15 y 2026-07-17 con Wompi ya cobrado — decidir si conciliar o reembolsar | P1 | S | MO-40 | — |
