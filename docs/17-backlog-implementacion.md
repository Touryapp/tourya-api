...
| BE-21 | **Asignar tours a `PROVIDER_OPERATOR`**. Al crear el sub-usuario (o al editarlo), el PROVIDER puede asignarle 1..N tours. Depende: nueva relación N:M `tour_operator` (tour_id, provider_operator_user_id, created_at). Nuevos endpoints `POST/DELETE /provider/users/{operatorId}/tours/{tourId}` + `GET /provider/users/{operatorId}/tours`. Restringir que el tour pertenezca al provider que hace la request. Origen: observación Luis 2026-07-17 en doc 05 §RN-007+ | P1 | M | — | Doc 05 (Luis 17-jul), Doc 03 |
| BE-22 | **Contacto principal por tour**. Sobre la N:M de BE-21 agregar `is_principal boolean`. Regla: por cada tour, exactamente 0 ó 1 operador principal. Endpoint `PUT /tour/{tourId}/principal-operator/{userId}` (idempotente, mueve el flag). Uso downstream: notificaciones y datos de contacto público del tour. Origen: observación Luis 2026-07-17 en doc 05 §RN-009+ | P1 | S | BE-21 | Doc 05 (Luis 17-jul) |
| BE-23 | **Job DIMAR bandera roja → cancelación masiva + créditos**. Trigger: al crear un `MaritimActivityReport` con flag=RED (síncrono en el mismo request), buscar `Reservation` donde `tour.category = report.category AND tour.subcategory = report.subcategory AND reservation_date BETWEEN report.start_date AND report.end_date AND delivery_status IN (PENDING, ...)`. Por cada match: cancelar reserva + crear `Credit` a nombre del turista por el monto pagado. Notificar por email + push (usa MO-40b eventos). Alternativa async: publicar `MaritimeAlertCreatedEvent` y procesar en listener `AFTER_COMMIT` para no bloquear el POST. Origen: observación Luis 2026-07-17 en doc 05 §RN-054 | P1 | S | MO-40b | Doc 05 (Luis 17-jul), RN-054 |
| BE-24 | **Reasignar reserva a otro Provider (ADMIN)**. Endpoint `PUT /admin/reservations/{id}/reassign` body `{newProviderId, reason}`. Validaciones: (a) el provider destino tiene al menos un tour ACEPTADO con la misma `subcategory`; (b) la reserva no está en estado terminal; (c) el provider original ya manifestó imposibilidad — nuevo `provider_declined_at` en `reservation`. Si no hay sustituto: endpoint alternativo `PUT /admin/reservations/{id}/cancel-with-credit` que cancela + genera crédito. Origen: observación Luis 2026-07-17 en docs 03 (rol ADMIN) + 05 (RN nueva) | P1 | M | — | Doc 03 + Doc 05 (Luis 17-jul) |
| BE-25 | **Confirmar cross-provider view del ADMIN**. Verificación (posiblemente ya cerrada): auditar que `GET /reservations` (o `sp_get_provider_reservations`) devuelva todas las reservas de todos los providers cuando lo llame un usuario con rol `ADMIN`, sin filtrar por `provider_id` del JWT. Si ya está: cerrar como no-op y documentar en RN. Si no: relajar el `WHERE` para ADMIN. Origen: observación Luis 2026-07-17 en doc 03 (nueva capacidad ADMIN) | P2 | XS | — | Doc 03 (Luis 17-jul) |
...
| FE-14 | **UI PROVIDER asignar tours a operador** — soporta BE-21 + BE-22. En la vista `/providers/users` extender el modal de crear/editar operador con: (a) multi-select de tours del provider; (b) marcador de "operador principal" por tour. Requiere textos i18n | P1 | S | BE-21, BE-22 | Doc 05 (Luis 17-jul) |
