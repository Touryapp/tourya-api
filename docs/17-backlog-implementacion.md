...
| BE-24 | **Provider Decline: cancelación automática + crédito + email alternativas** (rediseño Luis 2026-07-23 en issue #193). Ver **RN-055 actualizada** en doc 05. **Cambio importante vs versión previa**: se descartó la reasignación manual del ADMIN — ahora al declinar, el sistema automáticamente cancela + genera crédito (reusa BE-23) + envía email al turista con tours alternativos de la misma subcategoría. Fase 1 (arrancable): (a) migración `provider_declined_at TIMESTAMPTZ NULL` en `reservation`; (b) `PUT /provider/reservations/{id}/decline` que dispara el cancel-with-credit + envía email; (c) ajuste en `ProviderPayoutOrderService` para excluir reservas con `provider_declined_at IS NOT NULL` del payout del proveedor; (d) email template `provider-declined-notification.html` con lista de tours alternativos (query por subcategory + disponibilidad futura). Fase 2 (roadmap, futuro): sistema de penalizaciones por ventana temporal (48h/24-48h/<24h/no-show) con multas económicas, descenso en búsquedas y cupones al turista | P1 | M | — | RN-055 (doc 05) + issue #193 |
