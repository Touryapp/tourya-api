...
| FE-15b | **Reemplazo `isAdmin()` → `isTouryaBackoffice()` por pantalla**. Post-P2. Componentes con variable `isBackoffice` que hoy son alias de `isAdmin()`: `tours-detail.component.ts:227`, `provider-reviews.component.ts:44`, `provider-panel.component.ts` en el redirect a `/admin/bookings-management`. Depende del mapa de permisos que Luis confirme en P2 (¿sinónimo total del ADMIN o subset?). Si es sinónimo total: reemplazo directo. Si es subset: caso-por-caso | P1 | S | FE-15 | Doc 03 + P2 pendiente Luis |
