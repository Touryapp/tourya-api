...
| SEC-06 🟢 | Reemplazar Firebase social login por **Token Exchange** (Google + Facebook) — CERRADO 2026-08-30. **Fase A backend** desplegada desde 2026-07-11 (endpoints `POST /auth/google` + `POST /auth/facebook` validan tokens server-side contra Google API y Facebook Graph API). **Fase B web (FE-02)** mergeada 2026-08-30 (tourya-front PR #127 commit `d7ec8fe`): frontend Angular consume los endpoints Token Exchange nuevos; Firebase SDK removido completo (`npm uninstall firebase @angular/fire`, grep verifica cero matches). **Fase C mobile (MO-13)** en curso — mismo trabajo en MAUI (Google Sign-In WebView + Facebook Login WebView) para dejar de consumir el endpoint legacy `/auth/social-auth` desde el APK. Endpoint legacy sigue vivo `@Deprecated` para backward-compat mientras MO-13 no cierre. Vulnerabilidad C-6 (doc 12): **cerrada para web Google end-to-end**; **cerrada para web Facebook a nivel código pero bloqueada en dev.tourya.co hasta que Luis complete config en Meta App** (issue tourya-front #124 — sin re-deploy nuestro cuando complete) | P0 | M | — | C-6, RN-006, [social-login-google-facebook.md](../social-login-google-facebook.md) |
...
| FE-02 🟢 | Migrar login social: quitar Firebase SDK, integrar Google Identity Services + Facebook JS SDK — CERRADO 2026-08-30 (tourya-front PR #127 mergeado, commit `d7ec8fe`). **Approach elegido**: vanilla GIS + FB SDK cargados dinámicamente en `ngAfterViewInit` (cero dependencias npm nuevas — mismo diseño que la rama vieja `05fd6cf` que quedó como referencia). **11 archivos tocados**: `auth.service.ts` reescrito con `authenticateWithGoogle/Facebook()` que consumen `POST /auth/google` y `POST /auth/facebook`; `login-tourist.component.{ts,html}` con botones GIS oficial + Facebook JS SDK; `guest-info-modal.applySocialFallback()` lee de `authService.getUser()` en vez del Firebase User (TC-009 documentType flow preservado); `environment{,.prod,.test}.ts` con `googleClientId` + `facebookAppId` extraídos de `05fd6cf` (mismo project `tourya-169d6`); `app.module.ts` sin `initializeApp`; `package.json + lock` sin `firebase` ni `@angular/fire`; `social-login.dto.ts` eliminado. Rama vieja `feature/fase-1-fe02-remove-firebase` borrada del remoto — el diseño quedó reimplementado sobre develop actual respetando 121 commits de deriva (TC-023 i18n con `TranslateService` inyectado, TC-009 documentType, TC-020 refresh perfil, cambios shape del `AuthResponse`). Testing manual esperado: localhost Google/Facebook ✅, dev Google ✅, dev Facebook bloqueado por Luis. **Deuda menor**: `photoURL` fallback en guest-info-modal se pierde (Firebase User era la única fuente); item futuro chico agregar `photoUrl` al `SocialResponseDto` del backend si UX lo pide | P0 | M | SEC-06 | Doc 12 C-6 |
...
| MO-13 🚧 | Integrar login social sin Firebase (Google + Facebook nativos MAUI) — **en curso 2026-08-30**, rama `feature/mo-13-social-login-token-exchange` (worktree local, sin remoto Git por MO-00 diferido). Approach recomendado por el agente: WebView OAuth flow (mismo patrón que WompiHelper) para Google y Facebook — cero dependencias NuGet nuevas, cero Google Play Services binding (obsoleto en .NET 8+), consistente cross-platform. Config nueva en `Constants.SocialAuth` con `GoogleClientId` + `FacebookAppId` del project `tourya-169d6` + redirect URIs (custom scheme `com.tourya.mobile:/oauth2redirect` o similar). `AuthService.SocialAuthAsync` legacy (que consume `/auth/social-auth`) se remueve — no hace falta backward-compat mobile porque la app es controlada por nosotros. **Firebase Cloud Messaging (MO-40) intacto** — `Plugin.Firebase.CloudMessaging` sigue instalado solo para push. Reporta al cierre: config pendiente para Franklin en Google Cloud Console (redirect URI del custom scheme) + config pendiente para Luis en Meta App (redirect URI Facebook) | P1 | M | SEC-06 | Doc 15 |
...
| IA-08 🟢 | Agente 5: **Backoffice Support** backend CERRADO 2026-08-19 (rama `feature/ia-08-backoffice-support-backend`). Nuevo paquete `com.tourya.api.agents.backoffice` con `BackofficeSupportService` (guard rol ADMIN/BACKOFFICE_OPERATION vía `Utils.isTouryaBackoffice`, mismo patrón que IA-11). 4 endpoints action-specific bajo `/admin/agents/backoffice-support/*`: (1) `POST /kyb-checklist/{requestProviderId}` — checklist deterministico contra `RequestProviderDocumentType.getAllRequestProviderDocumentTypeList` filtrado por `mandatory=true` (RN-045) cruzado con `request_provider_gallery`; LLM opcional aporta `reasoning` + `overallStatus` (COMPLETE/INCOMPLETE/REJECTED — REJECTED cuando ≥3 CRITICAL). (2) `POST /tour-prevalidation/{tourId}` — RN-011 español obligatorio + RN-013 galería mínima 3 imágenes + RN-046 `cancellationPolicyType` seteado; deterministico, LLM solo aporta el resumen natural. Códigos estables: `MISSING_ES_NAME`, `MISSING_ES_DESCRIPTION`, `GALLERY_EMPTY`, `GALLERY_FEW_IMAGES`, `CANCELLATION_POLICY_MISSING`, `CANCELLATION_POLICY_TYPE_MISSING`. (3) `POST /dimar-draft?date=&providerId=` — **sin LLM**, agregación estructurada; query nativa nueva `ReservationRepository.findConfirmedForProviderOnDate(providerId, scheduleDate)` que une `reservation → shopping_cart_item → tour_schedule → tour` filtrando por `provider_id`, `schedule_date` y `delivery_status IN ('PENDING','DELIVERED')`; expande por `ageType` de `shopping_cart_item_detail` + `payment.payer_*`; RN-054 sigue 100% manual, el agente solo pre-arma. (4) `POST /payout-anomalies?from=&to=` — concilia cada `ProviderPayoutOrder` del rango contra `AccountPayable` esperado (RN-042 `amount al operador = providerPrice x quantity`); códigos `MISMATCH`, `MISSING_ACCOUNT_PAYABLE`, `TOTAL_DRIFT`; LLM opcional aporta explanation por anomalía. Guardrails idénticos a IA-02/IA-07: deny-list secretos (WOMPI_INTEGRITY_SECRET, WOMPI_EVENTS_SECRET, JWT_SECRET, ANTHROPIC_API_KEY, FIREBASE_ADMIN_SDK_JSON, GEMINI_API_KEY) → `escalated_to_human=true` sin llamar al LLM; scrub recursivo de `providerPrice`/`slotPercentageTourya`/`slotPorcentajeTourya`/`porcentajeTourya`/`providerUnitPrice`; `BudgetGuard.canRun("BackofficeSupport")` cap default $20/mes ya registrado en `AGENT_BUDGET_BACKOFFICESUPPORT_USD_MONTHLY` (migración 076). El agente NUNCA aprueba/rechaza/modifica — RN-010/RN-046/RN-042 lo blindan en código. Modelo Gemini 2.5 Pro (reusa infra GCP de IA-02/IA-07). **Sin migración BD** — reusa `agent_audit_log.metadata` JSONB. 13 unit tests con `MockLlmClient` (kyb COMPLETE/CRITICAL, tour MISSING_ES/GALLERY_FEW, dimar con reservas/sin reservas/audit sin costo, payout MISMATCH RN-042, guardrail deny-list, guardrail scrub, autorización 4 capabilities, 404 RequestProvider, computeOverallStatus REJECTED, defaultAnomalyExplanation) — 13/13 verdes. Cero cambios en `TravelConciergeService`, `OperatorSupportService`, `GoogleCloudTranslationService`, `AgentObservabilityService`, `AnthropicClient`, `GeminiClient`, `AgentAuditWriter`. Costo esperado ~$0.50-1/mes. Docs 00, 09, 16, 17 sincronizados | P2 | M | IA-01 | [16](16-agentes-ia.md) §Agente 5 |
| IA-08-FE 🟢 | Agente 5: **frontend admin Angular** — CERRADO 2026-08-19 (tourya-front PR #122 mergeado, commit `ff19c18`). 4 touchpoints en las páginas admin existentes: (1) **KYB Checklist** — botón `Checklist automático con IA` en el detalle de RequestProvider dentro de `dashboard/dashboard.component`, abre modal con `overallStatus` (COMPLETE/INCOMPLETE/REJECTED), tabla de docs con badges OK/WARN/CRITICAL, sección reasoning. (2) **Tour Prevalidation** — botón `Pre-validar con IA` en `tour-admin-detail.component`, modal con `canApprove`, tabla de issues ordenados por severity (CRITICAL/WARN/INFO) + reasoning. (3) **DIMAR Draft** — nueva página `/admin/dimar-draft` (`standalone: true`) con filtros date + `providerId` (input numérico — adaptación: el frontend no expone servicio de listado de providers, sería deuda futura), tabla de pasajeros, botón "Exportar CSV" client-side con BOM UTF-8 vía `String.fromCharCode(0xFEFF)`, botón "Imprimir" con `window.print()` + `@media print`. (4) **Payout Anomalies** — botón `Detectar anomalías con IA` en `provider-payments.component` (montado en `/admin/provider-payments`) con date-range picker; panel con anomalías + severity badges + explanation por anomalía + fallback silencioso. Nuevo `BackofficeSupportService` en `src/app/shared/services/` con 4 métodos + models en `src/app/shared/models/`. Sidebar link "DIMAR Draft" (ícono `isax-flag`) en `admin.component.html` **y** en `dashboard/dashboard.component.html` (defensivo — patrón MO-refactor #253). Namespace i18n `backofficeSupport.*` en es/en/pt. Build production verde, 0 warnings nuevos. **Nota flow merge**: PR corrió en paralelo con IA-10-FE — rebase manual sobre `develop` actualizado resolvió 7 conflictos (sidebar admin + dashboard + routing + 3 i18n files) manteniendo AMBOS items del sprint 13. **Cero cambios** en `travel-concierge.service.ts` / `operator-support.service.ts` / `agent-observability.service.ts` / `ConciergeChatWidget` / checkout / Wompi. Docs 00, 17 sincronizados | P2 | S | IA-08 | [16](16-agentes-ia.md) §Agente 5 |
...
| IA-10 🟢 | Agente 6: **Moderación de reseñas backend** CERRADO 2026-08-19 (rama `feature/ia-10-review-moderation-backend`). Nuevo paquete `com.tourya.api.agents.moderation` con `ReviewModerationService` + `ReviewModerationEvent` + `ReviewModerationEventListener` (`@TransactionalEventListener(AFTER_COMMIT)`) + `ReviewModerationApplier` (`@Async @Transactional`). Hook en `ReviewService.createReview` publica `ReviewModerationEvent(reviewId)`; el listener AFTER_COMMIT delega al método `@Async` fire-and-forget del service — la respuesta HTTP al turista NO espera el LLM (RN-050 se respeta: la reseña se publica igual, el agente solo flaggea metadata paralela). Migración `093_ia10_review_moderation.sql` agrega 4 columnas a `review` (`moderation_status VARCHAR(20)`, `moderation_flags JSONB`, `moderation_reasoning TEXT`, `moderated_at TIMESTAMP`) + índice parcial `idx_review_moderation_status WHERE IS NOT NULL` + GIN sobre `moderation_flags`. Veredictos: `APPROVED | PENDING | REJECTED`. Flags dominio cerrado: `SPAM | OFFENSIVE | OFF_TOPIC | POTENTIAL_FRAUD | INAPPROPRIATE_MEDIA`. Guardrails idénticos a IA-02/IA-07 (deny-list secretos → PENDING escalado sin LLM + budget guard `BudgetGuard.ReviewModerator` cap default $10/mes + modo no-op degradado si flag apagado + sanitize defensivo de flags fuera del dominio + fallback conservador APPROVED-con-flags → PENDING). Nuevos endpoints admin `POST /admin/agents/review-moderation/{reviewId}/re-moderate` (sync — devuelve `ModerationResult`) y `GET /admin/reviews/moderation?status=PENDING\|REJECTED&from&to&limit=50` (cola de revisión, `reviewText` truncado 200 chars) — ambos JWT ADMIN o BACKOFFICE_OPERATION. **Modelo `gemini-2.5-flash`** (cambio vs propuesta original que sugería Haiku 4.5 — Gemini Flash es equivalente en calidad de clasificación + más barato + reusa la infra Vertex AI productiva de IA-02/07). Costo esperado **$0.30-0.60/mes** con volumen actual. Nuevo `AdminReviewModerationQueryService` con JDBC (`_user` join + `tour` join + parsing JSONB `es`). Prompt versionado `prompts/review-moderation/moderate.v1.txt`. 17 unit tests con `MockLlmClient` cubriendo veredictos (APPROVED/REJECTED/PENDING), flags (SPAM/OFFENSIVE/OFF_TOPIC/POTENTIAL_FRAUD/INAPPROPRIATE_MEDIA), guardrails (deny-list secretos + budget exhausto + modo no-op + contradicción APPROVED+flags → PENDING + sanitize flags desconocidos + LLM error → PENDING escalado), applier persiste vía `applySync/applyAsync`, `comment` intacto (never modifies review text), verificación estructural de la anotación `@TransactionalEventListener(AFTER_COMMIT)` del listener, `ResourceNotFoundException` en reseña inexistente. Cero cambios en `TravelConciergeService` / `OperatorSupportService` / `GoogleCloudTranslationService` / `AgentObservabilityService` / `AnthropicClient` / `GeminiClient` / `AgentAuditWriter` — solo se agregan una línea en `ReviewService.createReview` (el `publishEvent`) y un import. Feature flags `AGENTS_MODERATION_ENABLED` + `AGENTS_MODERATION_MODEL`. Deuda futura: (a) IA-10-FE admin UI Angular para la cola, (b) IA-10b soporte vision para evaluar fotos con Gemini 2.5 Pro (hoy `INAPPROPRIATE_MEDIA` solo se dispara con evidencia textual). Docs 00, 05, 09, 16, 17 sincronizados | P2 | M | IA-01 | RN-050, [16](16-agentes-ia.md) |
| IA-10-FE 🟢 | Admin UI Angular para la cola de moderación de reseñas — CERRADO 2026-08-19 (tourya-front PR #121 mergeado, commit `f72a7bb`). Nueva página `/admin/reviews/moderation` (`standalone: true`, patrón `AgentsObservabilityComponent`) protegida por `BackofficeGuard` (ADMIN + BACKOFFICE_OPERATION). Consume `GET /admin/reviews/moderation` con filtros: tabs `PENDING` (default) / `REJECTED` + rango de fechas (`from`/`to` default últimos 30 días) + filtro por flag client-side (SPAM/OFFENSIVE/OFF_TOPIC/POTENTIAL_FRAUD/INAPPROPRIATE_MEDIA/todos). Tabla con: **Fecha** (moderatedAt), **Reseña** (reviewText truncado 200 chars con tooltip completo), **Tour** (tourName), **Autor** (authorEmail), **Flags** (chips coloreados por flag — SPAM amarillo / OFFENSIVE rojo / OFF_TOPIC gris / POTENTIAL_FRAUD naranja / INAPPROPRIATE_MEDIA púrpura), **Reasoning** (icono ⓘ con tooltip), **Acciones** (`Re-moderar` con loading + actualización optimista de la fila; `Ver reseña completa` abre modal con link a `/admin/reviews/{reviewId}` — deuda menor: esa ruta no existe todavía, cae al fallback). Nuevo `ReviewModerationService` en `src/app/shared/services/` con 2 métodos + models 6 tipos en `src/app/shared/models/` (`ReviewModerationSummaryDto`, `ModerationResult`, `ModerationFlag`, `ModerationStatus`, envelope `ApiResponse<T>`). Sidebar link "Cola de moderación" (ícono `isax-shield-tick`) en `admin.component.html` (shell) **y** en `dashboard/dashboard.component.html` (defensivo — patrón MO-refactor #253). Namespace i18n `reviewsModeration.*` en es/en/pt. **Sin emojis** en las traducciones (respeta rule del proyecto — íconos vía clases `isax-*`). Estados: loading spinner central, empty state, error banner con reintentar, aislado por sección. Build production verde 0 warnings nuevos. **Nota entorno**: el harness lanzó el worktree en `tourya-api` en vez de `tourya-front` — el agente adaptó creando worktree manual + junction a `node_modules`, trabajo entregado igual. Cero cambios en `travel-concierge.service.ts` / `operator-support.service.ts` / `agent-observability.service.ts` / `backoffice-support.service.ts` (que la otra rama IA-08-FE crea en paralelo) / `ConciergeChatWidget` / checkout / Wompi. Deuda menor: ruta `/admin/reviews/{reviewId}` (detalle admin de reseña) no existe — TODO para sprint futuro. Docs 00, 17 sincronizados | P3 | S | IA-10 | [16](16-agentes-ia.md) §Agente 6 |
...
| IA-11-FE 🟢 | Dashboard de observabilidad de agentes — **frontend Angular admin** CERRADO 2026-08-19 (tourya-front PR #120 mergeado, commit `8377723`). Nuevo `/admin/agents` con `BackofficeGuard` (ADMIN + BACKOFFICE_OPERATION) que consume los 6 endpoints del backend IA-11-BE. 6 secciones visuales: (1) summary cards por agente (calls/costo/tokens/avgLatency/successRate/escalatedRate); (2) timeseries chart con `ng-apexcharts` (ya en `package.json` — cero deps nuevas) — line chart calls + costo por día; (3) latency percentiles p50/p95/p99; (4) result types stacked bar; (5) top consumers table; (6) overrides con tooltip explicando el proxy MVP + deuda IA-11b. Filtros globales (rango fechas + selector agente + botón Actualizar). Nuevo `AgentObservabilityService` + 6 DTOs en `src/app/shared/`. Sidebar link "🤖 Observabilidad IA" en `admin.component.html` (shell) y en `dashboard/dashboard.component.html` (defensivo — bug MO-refactor #253). 20+ keys i18n `agentsObservability.*` es/en/pt. Loading spinners y error handling aislado por sección. Component `standalone: true` (patrón `AdminCreditsComponent`). Build production verde 0 warnings nuevos. Cero cambios en `travel-concierge.service.ts`, `operator-support.service.ts`, `ConciergeChatWidget`. Docs 00, 17 sincronizados | P2 | S | IA-11-BE | [16](16-agentes-ia.md) §Observabilidad |
