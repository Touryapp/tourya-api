...
| MO-40b | **Deuda hotfix #179**: mover hooks push del `PaymentService` (y demás flows Fase D que corren dentro de tx) a `@TransactionalEventListener(phase = AFTER_COMMIT)`. Objetivo: garantizar que ningún fallo del stack de push (DB, red a FCM, dependencias) pueda tumbar la transacción de negocio. | P1 | S | MO-40 | — |
...
| OP-179c | **Conciliación Wompi**: cruzar las 8 reservas TEMPORAL huérfanas (305-312, ventana 2026-07-15 → 07-17) contra el dashboard Wompi. Tras el deploy de INF-04 el job las va a marcar CANCELED en la próxima corrida — hay que confirmar si alguna corresponde a un cobro real y disparar refund manual. Los payer_id / payer_email del payment (si llegó a persistirse) o el link con `shopping_cart_item` sirven de puente | P1 | XS | INF-04 | — |
