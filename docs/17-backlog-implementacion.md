...
| MO-53 | **Reagendar reserva turista con 3 casuísticas (igual/menor/mayor precio)** — auditoría Sprint 5b (2026-08-14). `ReservationDetails` recibe `MaxReschedulingDate` + `CanReschedule` del backend pero el mobile los ignora; `IReservationService` no tiene método Reschedule; `ReservationDetailPage` no tiene botón "Reagendar". Backend RN-033 ya soporta los 3 casos (ver BE-20). Piezas: método `ReservationService.RescheduleAsync` (cliente de `PUT /reservations/{id}/reschedule`), `ReschedulePage` con selector de fecha bounded por `MaxReschedulingDate`, botón visible sólo si `CanReschedule = true` | P1 | S | BE-20 | Doc 15 §Post-Sprint 3/4a/5a |
| MO-54 | **Forzar cambio password primer login operario (`mustChangePassword`)** — auditoría Sprint 5b (2026-08-14). Security issue moderado: operario recién creado (via MO-24 `ResetPassword` del PROVIDER) usa clave temporal sin que el mobile lo fuerce a cambiarla. `AuthResponse` DTO no tiene `mustChangePassword`, `LoginViewModel.LoginAsync` navega al home sin chequear, no existe `ChangePasswordPage` ni endpoint self-service en `AuthService`. Piezas: agregar flag `mustChangePassword` a `AuthResponse` (requiere backend), nuevo `ChangePasswordPage`, redirección forzada desde `LoginViewModel` si el flag viene true, endpoint self-service `PUT /auth/change-password` en backend | P1 | S | — | Doc 15 §Post-Sprint 3/4a/5a |
