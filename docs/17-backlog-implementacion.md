...
| BE-24 | **Reasignar reserva a otro Provider (ADMIN)** — opción 3 (reasignar o cancelar con crédito) confirmada por Luis vía WhatsApp 2026-07-22. Ver **RN-055** en doc 05 con el proceso completo. Feature 100% nueva sin infra previa (auditoría 2026-07-19). Requiere: (a) migración `provider_declined_at TIMESTAMPTZ` en `reservation`; (b) `PUT /provider/reservations/{id}/decline` que el PROVIDER original invoca; (c) `PUT /admin/reservations/{id}/reassign` con body `{newProviderId, reason}` — valida `provider_declined_at NOT NULL`, provider destino con tour ACEPTADO misma `subcategory`, reserva no terminal; (d) `PUT /admin/reservations/{id}/cancel-with-credit` (reusa lógica BE-23); (e) notificaciones email al turista + push al nuevo provider. **Riesgo MEDIO**: reasignar cambia `tour_id` (o vínculo al slot) de la reserva — antes de tocar auditar consumidores (reporting, payouts, mobile del turista) | P1 | M | — | RN-055 (doc 05) + Luis WhatsApp 22-jul |
...
| FE-13 | **UI ADMIN reasignar reserva** — soporta BE-24, ver **RN-055** en doc 05. Modal en la vista admin de reservas con: (a) botón "Reasignar" habilitado solo si `providerDeclinedAt IS NOT NULL`; (b) dropdown de providers con tour ACEPTADO de misma subcategoría disponibles en la fecha/slot; (c) botón "Cancelar y generar crédito" si no hay sustitutos. Requiere textos i18n es/en/pt | P1 | S | BE-24 | RN-055 (doc 05) + Luis WhatsApp 22-jul |
