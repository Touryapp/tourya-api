...
| BE-22b | **UX del selector "Contacto principal" desde vista del tour** — solicitud Luis 2026-07-18 en RN-009 reescrita: *"por cada Tour, el PROVIDER debe especificar cuál de los `PROVIDER_OPERATOR` va a ser el contacto principal"*. Hoy la UI de Donnaly hace el flujo inverso (desde vista del operador se elige el tour). Trabajo: dropdown "contacto principal" desde vista del tour (Angular + mobile) que dispara `PUT /provider/users/{providerUserId}/principal-tour?tourId=X`. Pregunta abierta P1: ¿va en edit-tour, en detalle, o en ambos? | P1 | S | BE-22 | Doc 05 (Luis 18-jul) — P1 pendiente |
| BE-23 | **Job DIMAR bandera roja → cancelación retroactiva + créditos**. Auditoría 2026-07-19: existe `MaritimActivityReport` entity/service/repo + endpoint POST + enum `MaritimeFlagEnum {GREEN, YELLOW, RED}`. Uso hoy es DEFENSIVO (bloquea confirmar reservas nuevas en zona RED activa vía `ReservationService.java:1289`), NO retroactivo. Falta: hook al `create` que publique `MaritimeAlertCreatedEvent`, listener `@Async @TransactionalEventListener(AFTER_COMMIT)` que busque reservas del rango + subcategory + location, las cancele con motivo RAIN, cree `Credit` por reserva y notifique. Idempotencia: skipear si ya CANCELED. Reusa infra de MO-40b. Origen: observación Luis 2026-07-17 en doc 05 §RN-054 | P1 | S | MO-40b | Doc 05 (Luis 17-jul), RN-054 |
...
| FE-15 | **Angular reconocer rol BACKOFFICE_OPERATION**. Frontend hoy NO conoce el rol: `roles.enum.ts` solo tiene `USER=1, PROVIDER=2, ADMIN=3, PROVIDER_OPERATOR=34` — falta `BACKOFFICE_OPERATION`. `AuthService.isAdmin()` chequea solo `id === Roles.ADMIN`; variables `isBackoffice` en varios componentes son alias de `isAdmin()`. Trabajo mínimo (aditivo): agregar rol al enum + helpers `isBackofficeOperation()`, `isTouryaBackoffice()`. Reemplazo de `isAdmin()` por `isTouryaBackoffice()` donde representa "backoffice" queda condicionado a P2 pendiente (¿es sinónimo total del ADMIN o subset?) | P1 | S | BE-25 | Doc 03 + P2 pendiente Luis |
