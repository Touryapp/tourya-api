...
| BE-22b | **UX del selector "Contacto principal" desde vista del tour**. Luis (2026-07-19 WhatsApp, respuesta P1): *"podría ser un botón en el card del tour que despliegue la lista de PROVIDER_OPERATOR y se marca el principal ✅"*. Y aclara en RN-009: *"por default, el PROVIDER es el contacto principal de cada tour. En caso de que el PROVIDER haya creado sub-usuarios, puede colocar uno como contacto principal"* (comportamiento ya implementado por `TourPrincipalOperatorService.resolveForTour()` con fallback al provider). Trabajo: botón en el `<div class="card">` de cada tour en `tour-list` → abrir modal/dropdown con lista de operadores asignados + radio "principal" → dispara `PUT /provider/users/{providerUserId}/principal-tour?tourId=X`. Mobile: paridad opcional. Requiere primero **BE-22c** (endpoint listar operadores por tour) | P1 | S | BE-22, BE-22c | Doc 05 (Luis 18-jul), P1 confirmada Luis 19-jul |
| BE-22c | **Endpoint `GET /provider/tours/{tourId}/operators`** — necesario para BE-22b. Hoy `TourPrincipalOperatorService.resolveForTour()` devuelve solo EL PRINCIPAL. Falta un endpoint que devuelva TODOS los operadores asignados a un tour del PROVIDER (con flag `isPrincipal` por operador) para poblar el selector. Backend: nuevo método en repo `findByTourIdWithProviderUserAndUser`, nuevo endpoint que valida ownership del tour por el PROVIDER autenticado, devuelve `List<ProviderOperatorTourResponse>` | P1 | XS | BE-21 | Derivada BE-22b |
| BE-22d | **Nuevo campo `phone` en `PROVIDER_OPERATOR`** — solicitud Luis 2026-07-19 (WhatsApp, complemento P1): *"en la creación del PROVIDER_OPERATOR puedes agregarle un campo (teléfono), actualmente no lo tiene"*. Backend: migración agregar columna `phone VARCHAR` en `_user` o `provider_user` (evaluar cuál — `_user.phone` puede ya existir), agregar campo a `CreateProviderOperatorRequest` + `UpdateProviderOperatorRequest` + `ProviderOperatorResponse`. Frontend: campo `phone` en formulario create/edit operator (Angular + mobile). Impacto: usado por `TourPrincipalOperatorService.fromOperatorLink()` que ya expone `phone` pero hoy lo saca del `provider.getPhone()` (no del operador). Ver también RN-026 (Luis 19-jul): "al momento en que se genera la reserva, debe aparecer la información (nombre, teléfono y correo electrónico) del contacto principal del tour" | P1 | S | BE-21 | Doc 05 (Luis 19-jul WhatsApp) |
...
| BE-24 | **Reasignar reserva a otro Provider (ADMIN)**. Auditoría 2026-07-19: feature 100% nueva, sin infra previa. Requiere: (a) migración `provider_declined_at TIMESTAMPTZ` en `reservation`; (b) `PUT /provider/reservations/{id}/decline` que el PROVIDER original invoca; (c) `PUT /admin/reservations/{id}/reassign` con body `{newProviderId, reason}` — valida `provider_declined_at NOT NULL`, provider destino con tour ACEPTADO misma `subcategory`, reserva no terminal; (d) `PUT /admin/reservations/{id}/cancel-with-credit` (reusa lógica BE-23); (e) notificaciones. **Riesgo MEDIO**: reasignar cambia `tour_id` de la reserva — antes de tocar auditar consumidores (reporting, payouts, mobile del turista). Origen: observación Luis 2026-07-17 en docs 03 (rol ADMIN) + 05 (RN nueva) | P1 | M | — | Doc 03 + Doc 05 (Luis 17-jul) |
...
| FE-15b | **Subset BACKOFFICE_OPERATION en Angular** — Luis 2026-07-19 respuesta P2 opción C: *"Ver reservas (solo consulta), pagos (subir pagos), gestionar reporte de actividades marítimas"*. Trabajo por pantalla: (a) rutas admin de reservas + reviews visibles con `isTouryaBackoffice()`; (b) `PayoutsPage` y flujo de subir comprobantes visible con `isTouryaBackoffice()` — hoy solo ADMIN puede marcar `PAID`, revisar si backend permite BACKOFFICE_OPERATION o hay que agregar (auditar primero); (c) módulo de gestión de `MaritimActivityReport` visible con `isTouryaBackoffice()`. NO habilitar para BACKOFFICE: aprobar/rechazar tours, editar comisión Tourya, activar/desactivar códigos de descuento del doc 18. Requiere: auditar cada endpoint que hoy es "solo ADMIN" para saber si acepta BACKOFFICE_OPERATION o hay que ampliar en `SecurityConfig` | P1 | S | FE-15 | Doc 03 + P2 confirmada Luis 19-jul (opción C) |
| BE-26 | **Bug reportado por Luis (2026-07-19 doc 05 §RN-026)**: reservas con tour de tipo de precio `GRUPO` fallan tras el pago Wompi con mensaje `"Error en la reserva. el pago fue exitoso pero hubo un error creando la reserva. Por favor contacte soporte"`. Mensaje distinto al 500 crudo de #179 → es bug controlado, específico de la lógica GRUPO. Código relacionado: `PaymentService.processPayment` + `ReservationService.java:1786/1853/2326` ("Para tours GRUPO: el precio es fijo independientemente de la cantidad"). Antes de tocar: pedirle a Luis un ejemplo reproducible en dev (tour + cantidad + carrito) + agarrar el stacktrace real de Cloud Logging. Si el mensaje ya no aparece (Luis lo escribió antes del fix de #179), cerrar como no-op | P1 | S | — | Doc 05 §RN-026 (Luis 19-jul) |
...
| FE-13 | **UI ADMIN reasignar reserva** — soporta BE-24. Modal en la vista admin de reservas con: (a) botón "Reasignar" habilitado solo si `providerDeclinedAt IS NOT NULL`; (b) dropdown de providers con tour ACEPTADO de misma subcategoría; (c) botón "Cancelar y generar crédito" si no hay sustitutos. Requiere textos i18n es/en/pt | P1 | S | BE-24 | Doc 03 + Doc 05 (Luis 17-jul) |
