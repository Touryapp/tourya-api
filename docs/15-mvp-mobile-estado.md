...
> **Nota 2026-09-18**: se agregó la sección "Post-Ciclo 2 QA — cierre 2026-09-18" al final. **Repo mobile ahora tiene remoto GitHub** (`Touryapp/tourya-mobile`) desde 2026-08-31 — MO-00 cerrado, la nota original de "sin remoto Git" queda histórica. **APKs v9→v11 distribuidos vía Firebase App Distribution** al proyecto `tourya-169d6` con Luis (`@wass.com.co` + `@tourya.co`) y Franklin (`@hotmail.com`). **3 sprints mobile cerrados en el ciclo**: #11 Admin panel B (3 páginas: reservas cross-provider + moderación reseñas + reportes DIMAR + nuevo `admin` TabBar en `AppShell`), #12 Provider Profile (`ProviderProfilePage` con datos empresa + KYB + logout + switch role, cierra queja Luis "Provider no tiene logout"), #10 Register provider single-page wizard. **APK v11** agrega tab Perfil admin (mismo patrón que Provider), cerrando la queja gemela de logout admin. **Ciclo 2 QA Luis** (matriz TCM #1): 12/12 TCs OK con v10. TCM-010 (Facebook login) desbloqueado por fix backend Secret Manager (`tourya-facebook-app-id` v2 con la app real de Luis `1572035247680930` en vez de la fantasma legacy `3998716940417210`) — Luis validó. **Fixes backend derivados del QA mobile**: PRs #279 (email branding v1 con logo), #280 (Sentry TOURYA-MOBILE-4 device-token audit type mismatch LocalDateTime vs OffsetDateTime), #281 (email branded v2 replicando look del frontend web), #282 (feedback Luis: quitar código destacado del email), #283 (Sentry TOURYA-MOBILE-5 `/reservations/date/{date}` LocalDate parse + range query). **Filas afectadas de la matriz Gap Analysis actualizadas** — Admin panel + Provider Profile pasan ⚠️ → ✅. Deudas abiertas: refinamientos futuros del email branding (10 templates restantes esperando OK de Luis sobre el piloto `activate_account_*.html`) + issue mobile #8 (piloto branded en review).
