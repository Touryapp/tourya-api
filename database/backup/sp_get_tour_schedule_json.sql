...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- LÓGICA CONDICIONAL: ESTRUCTURA COMPLETA O SIMPLIFICADA
    'schedules', CASE
      -- Si se busca un tour específico, mostrar la estructura completa de horarios
      ...
      -- Si no se busca un tour específico, mostrar solo el precio más alto
      ELSE
        (
          SELECT jsonb_agg(
            jsonb_build_object(
  ...
  WHERE
    ...
    AND (
      ...
      t.name ILIKE '%' || v_text_search || '%' OR
      t.description ILIKE '%' || v_text_search || '%'
    )
    AND (
      v_min_price IS NULL
      OR EXISTS (
