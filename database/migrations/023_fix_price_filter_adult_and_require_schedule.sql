-- Migration: Fix price range filter to use ADULT and require schedules in date range
-- Date: 2026-03-31
--
-- Problems fixed:
-- 1) minPrice/maxPrice filters were evaluated against ANY age_type (not adult) and required schedules,
--    causing unexpected empty results.
-- 2) Search could return tours with schedules = [] even when startDate/endDate were provided.
...
BEGIN
  ...
  WHERE
    ...
    -- Require schedules in date range when start/end provided
    AND (
