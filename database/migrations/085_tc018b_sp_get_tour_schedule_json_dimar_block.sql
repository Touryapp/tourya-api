-- Migration 085: TC-018 (#227) Bug B — expone blockedByMaritimeReport en sp_get_tour_schedule_json
-- Date: 2026-08-09
-- Description:
--   Luis pidio en #227 bloquear la posibilidad de agregar tours al carrito cuando
--   existe un reporte marItimo con flag=RED activo para la subcategoria + ubicacion
--   del tour cuyo rango de fechas contenga el scheduleDate.
--
--   Esta migracion agrega un flag booleano `blockedByMaritimeReport` a cada schedule
--   del jsonb de salida, calculado con EXISTS contra `maritim_activity_report`.
--   El frontend usa este flag para deshabilitar el dia en el modal de seleccion.
--   El backend Java agregara un guard duro en ShoppingCartService.addItemToCart para
--   rechazar el add si un cliente logra bypassear el frontend.
--
--   Se copia la firma exacta del SP anterior (037) y se agrega el campo dentro del
--   jsonb_build_object de cada schedule. Sin otras modificaciones al SP.
    ...
    'schedules', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'status', s.status,
          -- TC-018 (#227) Bug B: flag para el frontend — si es true, el modal deshabilita
          -- el dia. Backend duplica el guard en ShoppingCartService por seguridad.
