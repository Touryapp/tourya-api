-- YA EJECUTADA EN EC2 (2026-06). No volver a correr si aplicaste 061.
-- Versión histórica: fallback de providerPrice en el SP (sustituida por 060 backfill + 061 cleanup).

DROP FUNCTION IF EXISTS public.sp_get_provider_reservations(int4, int8, varchar);
...
BEGIN
    ...
    SELECT
        ...
        COALESCE((
            SELECT SUM(
                COALESCE(
                    NULLIF(scid.provider_unit_price, 0),
                    tscp.provider_price,
                    CASE
                        WHEN COALESCE(so.slot_porcentaje_tourya, 0) > 0
                        THEN ROUND(scid.unit_price / (1 + so.slot_porcentaje_tourya), 2)
                        ELSE scid.unit_price
                    END
                ) * COALESCE(scid.quantity, 0)
            )
            FROM shopping_cart_item_detail scid
            LEFT JOIN tour_schedule_config_price tscp
                ON tscp.slot_id = sci.slot_id
               AND tscp.age_type = scid.age_type
            LEFT JOIN tour_schedule_slot_override so
                ON so.schedule_id = ts.id
               AND so.slot_id = sci.slot_id
