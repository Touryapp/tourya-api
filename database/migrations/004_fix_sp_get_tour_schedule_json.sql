-- =====================================================
-- Migration: 004_fix_sp_get_tour_schedule_json.sql
-- Description: Corregir error de columnas inexistentes min_capacity/max_capacity en SPs
-- Author: System
-- Date: 2026-02-16
-- =====================================================

-- =====================================================
-- 1. Corregir get_templates_by_provider
-- =====================================================
            ...
            'slots', (
                SELECT jsonb_agg(
                    jsonb_build_object(
                        ...
                        'endTime', s.end_time,
                        -- Eliminadas referencias a s.min_capacity y s.max_capacity que no existen en tour_schedule_config_slot
                        'prices', (
                            SELECT jsonb_agg(
                                jsonb_build_object(
                                    'id', p.id,
...
-- =====================================================
-- 2. Corregir sp_get_tour_schedule_json
-- =====================================================

-- Eliminar función previa para evitar conflictos de firma
                ...
                'slots', COALESCE((
                  SELECT jsonb_agg(
                    jsonb_build_object(
                      ...
                      'endTime', sl.end_time,
                      -- Eliminadas referencias a sl.min_capacity y sl.max_capacity que no existen
                      'prices', COALESCE((
                        SELECT jsonb_agg(
                          jsonb_build_object(
                            'ageType', p.age_type,
